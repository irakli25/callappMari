...
import { kendo_SelectedID, kendo_setIndex, kendo_setValueByID, kendo__refresh } from "../../helpers/kendo.helper.js";
...
export default class inputListModal extends Tdg {
    ...
    build = async () => {
        ...
        this.append(this.state.dom, this.state.element.bar);
        ...
        this.append(this.state.element.divSeparatedDemoView, await this.inputDemoArea());

    }
    deleteInput = () => {
        var selectedIDS = self.Automator.selectedKendoRows(self.inputListModal.state.element.InputsByFieldsetTable);
        self.Automator.getResponse({
            ...
            kendo__refresh(self.inputListModal.state.element.InputsByFieldsetTable, 'table');
            self.inputListModal.buildNotice({msg: "ველი წაშლილია"});
    ...
    createNewInput = (cb) => {
        this.buildModal(null,  {
            width: '22vw',
            height: '50vh',
            content: new inputModal(cb).init(),
            buttons: {
                save: {
                    name: "შენახვა",
                    onclick: function(modal){ 
                        self.Automator.getResponse({
                            route: "Automator",
                            act: "saveInput",
                            field_id: self.inputListModal.state.id,
                            input_id: cb.id,
                            input_type: kendo_SelectedID(self.inputModal.state.selectorkendo['input_type']),
                            input_name: $(self.inputModal.state.element.inputName).find('input').val(),
                            input_tab: kendo_SelectedID(self.inputModal.state.selectorkendo['input_tab']),
                            input_pos: $(self.inputModal.state.element.inputPosition).find('input').val(),
                            key: self.inputModal.state.inputKey,
                            nec: kendo_SelectedID(self.inputModal.state.selectorkendo['necessary'])

                        }).then(function (data){
                            
                            kendo__refresh(self.inputListModal.state.element.InputsByFieldsetTable, 'table');
                            self.inputListModal.buildNotice({msg: "ველი შენახულია"});
                            modal.close().destroy();
                            //kendo__refresh(self.Incomming.state.element.table, 'table');
                        })
                    },
    ...
    createFieldSet = (field, i) => {
        ...
        this.append(this.state.element.demoViewFieldset, this.state.fieldset[fieldsetName])
    ...
    createInput = (x) => {
        ...
        this.state.inputs.fieldsetNameInput = new Input({
            type: "text",
