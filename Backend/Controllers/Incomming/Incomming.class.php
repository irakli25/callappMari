...
class Incomming extends dbClass
{
    ...
    public function getList()
    {
        ...
        if (!empty($operator)) {
            $filter .= " AND IFNULL(user_info.user_id, user_info2.user_id) = '$operator'";
        }
        ...
        if(!in_array($_SESSION['GROUPID'], $arr)){
            $ex = "AND IFNULL(user_info.user_id, user_info2.user_id) = '$_SESSION[USERID]'";
        }else{
            $ex = '';
        }
        ...
        parent::setQuery("   SELECT	    incomming_request.id,
                                        IFNULL(user_info.user_id, user_info2.user_id) AS operator_id,
                                        source.key,
                                        incomming_request.chat_id,
                                        incomming_request.ussd_id,
                                        incomming_request.datetime,
                                        IFNULL(user_info.name, user_info2.name) AS operator,
                                        IFNULL(processing.`value`->>'$.inputs.telefoni___987',incomming_request.request_name) AS client,
                                        asterisk_call_log.did AS rigi,
                                        IF(incomming_request.source_id = 12, ussd.CustomerFullName , processing.`value`->>'$.inputs.abonenti___079') AS abonenti,
										processing.`value`->>'$.inputs.abonentis_nomeri___019' AS abonentis_nom,
                                        ussd.CustomerNumber AS abonentis_nom_ussd,
                                        '' AS abonentis_nomeri,
                                        IFNULL(category.name,cat2.name) AS cat,
                                        IFNULL(category1.name,cat3.name) AS qvecat,
                                        ...
                                        asterisk_queue.number as `queue_number`
                                ...
                                LEFT JOIN	user_info ON user_info.user_id = users.id
                                LEFT JOIN	user_info as user_info2 ON user_info2.user_id = asterisk_call_log.user_id
                                ...
                                LEFT JOIN 	info_category_114 AS cat2 ON cat2.id = processing.`value`->>'$.multilevel.zaris_kategoriebi_114___726___1'
                                LEFT JOIN 	category AS category1 ON category1.id = processing.`value`->>'$.multilevel.zaris_kategoriebi___594___3'
								LEFT JOIN 	info_category AS cat3 ON cat3.id = processing.`value`->>'$.multilevel.zaris_kategoriebi_114___726___3'
                                LEFT JOIN   source ON source.id = incomming_request.source_id
                                WHERE 	    incomming_request.actived = 1 AND (asterisk_call_log.call_status_id > 2 OR ISNULL(asterisk_call_log.id)) AND (asterisk_call_log.call_type_id = 1 OR ISNULL(asterisk_call_log.id)) $filter 
                                ORDER BY    id DESC");
    ...
    public function get_parent_id()
    {
        $parend_ids = [];
        $id = $_REQUEST['id'];
        parent::setQuery("SELECT parent_id FROM `category` WHERE id = '$id'");
        $parend_ids['second'] = parent::getResultArray()['result'][0]['parent_id'];

        if ($parend_ids['second'] > 0) {
            parent::setQuery("SELECT parent_id FROM `category` WHERE id = '$parend_ids[second]'");
    ...
    {
        $id = $_REQUEST['id'];
        parent::setQuery("SELECT id,name FROM `category` WHERE parent_id = '$id'");
    ...
    public function getChatTemplate(){
        ...
        parent::setQuery("SELECT `id`, `name`, `comment` FROM `chat_template` WHERE `actived` = 1");
        $data = parent::getResultArray()['result'];
        return $data;
